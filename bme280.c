...
int _bme280_setByte(byte reg, byte data){
		...
		return !i2c_write_timeout_us(I2C_PORT,I2C_bme280,config,2,true,600);
	#endif
}
...
uint16_t _bme280_getReg(byte reg){
		...
		i2c_write_timeout_us(I2C_PORT,I2C_bme280,&reg,1,true,600);
		sleep_ms(1);
		i2c_read_timeout_us(I2C_PORT,I2C_bme280,&data,1,false,600);
		return (int)data;
	#endif
}
